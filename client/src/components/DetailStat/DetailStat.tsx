          ...
          style={
            detailVisible === true
              ? {
                  height: `${
                    (stat / 100) *
                    (statBarContainer.current?.offsetHeight as number)
                  }px`,
                }
              : { height: "0px" }
          }
  ...
  return (
    <div className={styles.wrapper}>
      <button
        id={styles.left}
        onClick={() => {
          statContainer.current?.scrollBy(-100, 0);
        }}
      >
      ...
      <div className={styles.statContainer} ref={statContainer}>
        <ul>
          {arr.map((vote, idx) => {
            let stat = 0;
            if (
              detailResutData.계[searchThroughArr(regionArr, detailRegion)] !==
              0
            ) {
              stat =
                Math.floor(
                  (vote.vote[searchThroughArr(regionArr, detailRegion)] /
                    detailResutData.계[
                      searchThroughArr(regionArr, detailRegion)
                    ]) *
                    10000
                ) / 100;
            }
